...
import {
  NoteSharingContractArtifact,
  NoteSharingContract,
} from "../artifacts/NoteSharing.ts";
import {
  createAccount,
...
describe("E2E Shared Note", () => {
  ...
  describe("create_and_share_note(...)", () => {
    ...
    it("should revert if the note already exists", async () => {
      ...
      await expect(txReceipt).rejects.toThrow(/Note already exists/);
    });
  });
  ...
  describe("bob_action", () => {
    ...
    it("should revert if the note doesnt exist", async () => {
      ...
      await expect(txReceipt).rejects.toThrow(/Note not found/);
    });
  ...
  describe("alice_action", () => {
    ...
    it("should not revert", async () => {
      const txReceipt = await sharedNoteContract
        .withWallet(alice)
        .methods.alice_action(bob.getAddress())
        .send()
        .wait({ debug: true });
